...
export async function getCoursesByUser(userId) {
  ...
  const userCourses = await UserCourse.find({ user: userId })
    .populate("course")
    .select("course actif status -_id");

  // Filtrage des courses qui sont null
  const cleanedUserCourses = userCourses.filter((uc) => uc.course !== null);

  return cleanedUserCourses;
}
