...
export default function ShowUserPage() {
          ...
          usersSansAdmin.map(async (user) => {
            ...
            try {
              ...
              const cleanedCourses = courses.filter(
                (c) => c && c.course && typeof c.course._id === "string"
              );

              return { ...user, courses: cleanedCourses };
            } catch (err) {
              console.error(`courses loading error for ${cleanUserId}:`, err);
              return { ...user, courses: [] };
            }
          })
        );
  ...
  return (
    <>
      <Navbar
        links={[
          { value: "Dashboard Admin", hasDropdown: false, path: "/dashboard" },
        ]}
      />
      <div
                  ...
                  >
                    {user.courses.length === 0 ? (
                      ...
                      user.courses.map((course) => (
                        ...
                        >
                          {course.course && course.course.logo && (
                            <img
                              src={`${
                          ...
                          >
                            {course.course && <p>{course.course.title}</p>}
                          </h4>
